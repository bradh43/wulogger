...
def null_safe_field(dictionary, field):
...
def put_new_athlete(input_data):
	...
	shoeList = {'L': [{'M': get_shoe_info(shoe)} for shoe in input_data['shoe_list']]}
...
def get_activity(activity):
	return dict(
		...
		shoe = {'S' : null_safe_field(activity, 'shoe')}
	)
...
def put_new_log(input_data):
	...
	postItem = dict(
		...
		activity_list={'L': [{'M' : get_activity(activity)} for activity in input_data['activity_list']]},
		like_list={'SS': input_data['like_list']},
		comment_list={'L': [{'M' : get_comment(comment)} for comment in input_data['comment_list']]}
	)
...
if __name__ == '__main__':
	put_new_athlete(athlete_info)
	put_new_log(log_info)
